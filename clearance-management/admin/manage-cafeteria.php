...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("UPDATE cafeteria_clearance SET status=? WHERE id=?");
    $stmt->bind_param("si", $status, $id);
    $stmt->execute();
    header("Location: manage-cafeteria.php");
    exit();
}

$result = $conn->query("SELECT * FROM cafeteria_clearance ORDER BY id DESC");
?>

<?php include 'includes/menu.php'; ?>

<div style="max-width:900px; margin:40px auto; background:#f8f8f8; padding:20px; border-radius:8px; color:#333;">
    <h2 style="text-align:center; color:#008B8B;">Manage Cafeteria Clearance</h2>
